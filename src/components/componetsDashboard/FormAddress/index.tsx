...
const FormAddress = () => {
  ...
  const [idDepartment, setIdDepartment] = useState<string>(!id ? '9' : address?.department.id as string);
  ...
  const handleForm: SubmitHandler<AddressForm> = (data, event) => {
    ...
    if (data.city !== 'Cargando...') {
      const newAddress: Address = {
        ...
        address: `${data.type} ${data.typeValue} #${data.number1}-${data.number2}`,
        department: {
          name:
  ...
  useEffect(() => {
    ...
    if(id && idDepartment === address?.department.id){
      fetch(`https://api-colombia.com/api/v1/Department/${address?.department.id}/cities`)
      .then((res) => res.json())
      .then((data) => setCities(data))
      .then(() => setLoadingCities(false))
    } else {
      fetch(`https://api-colombia.com/api/v1/Department/${idDepartment}/cities`)
      .then((res) => res.json())
      .then((data) => setCities(data))
      .then(() => setLoadingCities(false))
    }
          ...
          <tr className="flex items-center justify-between p-2 py-4 bg-primary-dm gap-x-0.5 text-sm">
            <td className="w-32 flex items-center justify-center">
              ...
              >
                {address?.department.id === idDepartment && (
                  <option value={[`${address?.department.id},${address?.department.name}`]}>
                    {address.department.name}
                  </option>
                )}
                {loading && <option>Cargando...</option>}
                {id && departments?.filter((department) => department.name !== address?.department.name).map((department) => (
                  <option
                    key={department.id}
            ...
            <td className="w-36 flex items-center justify-center">
              ...
              >
                ...
                {idDepartment === address?.department.id && address?.city && <option value={address.city} defaultChecked>{address.city}</option>}
                {id && cities?.filter((city) => city.name !== address?.city).map((city) => (
                  <option
                    key={city.id}
                    value={city.name}
                    defaultChecked={city.name === address?.city ? true : false}
                  >
                    {city.name}
                  </option>
                ))}
                {!id && cities?.map((city) => (
                  <option
                    key={city.id}
                    value={city.name}
                    defaultChecked={id ? city.name === watch('department') && true : false}
                  >
                    {city.name}
                  </option>
                ))}
